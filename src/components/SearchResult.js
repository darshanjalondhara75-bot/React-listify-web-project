...
import { Input, Empty, Radio, Space } from 'antd';
...
const mockProducts = [
  {
    ...
    price: 1200000,
    images: ['https://placehold.co/100x100?text=Ad+Image'],
    status: 'active',
    category: { _id: 'cat1', name: 'Mobile Phones' },
    location: { _id: 'loc1', name: 'Lagos, Nigeria' },
    condition: 'New',
    author: { _id: 'auth1', name: 'David John', username: 'davidjohn' },
  ...
  },
  {
    ...
    condition: 'Used',
    author: { _id: 'auth2', name: 'Liam Thompson', username: 'liamt' },
    createdAt: new Date().toISOString()
  },
  {
    _id: '3',
          ...
          <div className='card rounded-0 profile-card card-shadow'>
            ...
            {searchResult.length > 0 && (
              <div className='card-body desktop-product-view'>
                ...
                <Pagination
                  count={Math.ceil(searchResult.length / countPerPage)}
                  page={current}
                  onChange={(event, value) => loadSearchResults(value)}
                  variant="outlined"
                  shape="rounded"
                  color="error"
                  sx={{
